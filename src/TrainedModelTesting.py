...
def GUIcall(results):
    ...
    c = Thread(target=label_image.classify, args=("hand_images/TestFiles/current_frame.jpg",))
    ...
    while (1):
        cv2.imwrite("hand_images/TestFiles/GUI_display.png", frame)
        ...
        cv2.imwrite("hand_images/TestFiles/GUI_display.png", frame)
        cv2.imwrite("hand_images/TestFiles/bitwise_hand.png", motion)

        #save the frame in order to test it
        if c.isAlive() != True:
            cv2.imwrite("hand_images/TestFiles/current_frame.jpg", motion)
            c = Thread(target=label_image.classify, args=("hand_images/TestFiles/current_frame.jpg", q))
            c.start()

        try:
            results.put(q.get(False))
        except queue.Empty:
            pass
